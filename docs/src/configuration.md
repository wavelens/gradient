...
| `settings.evalForkWorkers` | `null` | Number of parallel eval subprocesses in the pool. When null, auto-sizes to the host core count capped at 16, then further bounded by `maxEvalRss` vs host RAM. Currently eval runs **serialized to one worker** regardless (concurrent workers deadlock on the shared eval-cache); the per-system sharding still bounds memory by recycling sequentially. (`GRADIENT_EVAL_FORK_WORKERS`) |

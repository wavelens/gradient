...
| `settings.evalForkWorkers` | `4` | Number of warm fork children the eval subprocess runs in parallel (`GRADIENT_EVAL_FORK_WORKERS`) |
| `settings.maxEvalRss` | `2147483648` (2 GiB) | Kill + re-fork an eval child once its RSS exceeds this many bytes (`GRADIENT_MAX_EVAL_RSS`) |

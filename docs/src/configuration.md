...
| `graphConsistencyIntervalSecs` / `GRADIENT_GRAPH_CONSISTENCY_INTERVAL` | 300 | Build-graph consistency sweep interval; violations log as warnings. The sweep also repairs `cached_path.missing_references` over the paths pending anchors gate on and is that counter's only backstop, so 0 disables both. |

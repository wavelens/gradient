...
| `settings.workerHeartbeatTimeoutSecs` | `30` | Seconds a connected worker may go silent before the server declares it dead and re-queues its in-flight jobs. The worker heartbeats every 10 s, so the default tolerates three missed beats. This is the only detector for a worker that dies without a clean TCP close (hard OOM-kill, frozen host, network partition); a graceful disconnect is handled immediately regardless. `0` disables the liveness watchdog. (`GRADIENT_WORKER_HEARTBEAT_TIMEOUT_SECS`) |
| `upstreamQueryConcurrency` / `GRADIENT_UPSTREAM_QUERY_CONCURRENCY` | 32 | Max simultaneous outbound upstream narinfo probes server-wide (latency ordering). |

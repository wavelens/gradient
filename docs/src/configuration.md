...
| `settings.smallNarBytes` | `1048576` (1 MiB) | A NAR at or under this many bytes is relayed through the server on upload, pulled through it on download and admitted to the hot RAM cache; larger NARs keep their presigned S3 URLs. (`GRADIENT_SMALL_NAR_BYTES`) |
...
| `settings.narUploadGraceHours` | `24` | Grace before the orphan-files GC reclaims a NAR object no DB row references (covers the upload commit window). Also the age past which an unconfirmed `cached_path` with neither a staged file nor an object is demoted by the `nar-uploader`, and the age past which a completed build that never backed one of its outputs is failed `OutputMissing` rather than waiting for a commit still in flight. (`GRADIENT_NAR_UPLOAD_GRACE_HOURS`) |

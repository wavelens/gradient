...
- **Stats line**: CPU time, builds, cache size, workers busy and queue wait across your projects (all projects for superusers).
...
- **Task table**: the top 10 tasks, "Show all" pages through the rest 25 at a time. Each row shows the star, `project / task` with the latest commit and its age, the last evaluations, entry points ok/total, the delta against the previous evaluation, speed, reliability and evaluations per week.
...
- **Rail**: your projects and caches, starred first. Starred and active projects list their tasks. Superusers and worker owners also get Operations links (Job Board, Workers, Scheduler, Health).
...
- The task table covers the projects you are a member of plus everything you starred.
- Active means a non-PR evaluation in the last 14 days.
- Tiers, in order:
    1. starred and active
    2. active
    3. starred (also a starred public project you are not a member of)
    4. member only
...
- Stars are personal: they only rank and filter what you see (tiers, the Starred chip, the rail order, palette results).

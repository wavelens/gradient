...
- **Workers** - fleet over time (connected vs draining), capability trend, three load radars (by **capability**, **architecture**, and **feature**), per-worker slot utilisation, and the live worker table. Each load radar plots busy % as the in-flight jobs of that kind against the summed slot capacity of the workers that can serve it (`GET /api/v1/board/workers/load`), so an operator can tell whether the fleet is eval-, build-, or architecture-bound and which architecture/feature needs more workers.
- **Cache** - cache totals, traffic, and storage-growth series (`GET /api/v1/board/cache`), plus per-upstream latency. Upstream metrics are keyed by URL, so the same URL registered under several caches/orgs shows as one series.

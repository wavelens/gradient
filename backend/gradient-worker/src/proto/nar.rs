...
/// the worker restarts from offset 0 (issue #225).
fn push_stream_token(level: i32) -> String {
        ...
        NarSource::Path { store } => {
            ...
            let meta = match sink {
                NarSink::Relay { nar_recv } => {
                    ...
                    stream_path_to_relay(job_id, store_path, writer, nar_recv).await?
                }
                NarSink::Presigned {
                    url,
                    method,
                    ...
                    let (compressed, meta) = pack_compress_path(store_path).await?;
    ...
    async fn open(
        ...
        writer
            .send(ClientMessage::NarStreamHeader {
                ...
                total_bytes,
                stream_token: push_stream_token(NAR_ZSTD_LEVEL),
            })
            .await?;

...
async fn run_retention(state: &Arc<ServerState>) {
    ...
    if cfg.metrics_retention_raw_days > 0 {
        ...
        if let Err(e) = entity::phase_event::Entity::delete_many()
            .filter(entity::phase_event::Column::At.lt(cutoff))
            .exec(db)
            .await
        {
            warn!(error = %e, "phase_event retention failed");
        }
        if let Err(e) = entity::worker_sample::Entity::delete_many()
            .filter(entity::worker_sample::Column::At.lt(cutoff))
            .exec(db)
            .await
        {
    ...
    if cfg.dispatch_retention_days > 0 {
        ...
        if let Err(e) = entity::dispatched_job::Entity::delete_many()
            .filter(entity::dispatched_job::Column::CreatedAt.lt(cutoff))
            .exec(db)
            .await
        {
    ...
    if cfg.metrics_retention_rollup_days > 0 {
        ...
        if let Err(e) = entity::metric_rollup::Entity::delete_many()
            .filter(entity::metric_rollup::Column::BucketStart.lt(cutoff))
            .filter(entity::metric_rollup::Column::Granularity.is_in([GRAN_MINUTE, GRAN_HOUR]))
            .exec(db)
            .await
        {

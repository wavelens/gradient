...
mod tests {
    ...
    fn test_cli_defaults() {
        let cli = Cli {
            ...
            oauth_scopes: None,
            oidc_discovery_url: None,
            crypt_secret_file: "test".to_string(),
            jwt_secret_file: "test".to_string(),
            serve_cache: false,
            binpath_nix: "nix".to_string(),
            binpath_git: "git".to_string(),
            binpath_zstd: "zstd".to_string(),
        ...
        assert_eq!(cli.max_concurrent_evaluations, 10);
        assert_eq!(cli.max_concurrent_builds, 1000);
        assert_eq!(cli.evaluation_timeout, 10);
        assert!(!cli.oauth_enabled);

...
pub async fn prefetch_inputs(
    ...
    // ── Enumerate the .drv's direct input store paths ────────────────────────
    let full_drv_path = nix_store_path(&task.drv_path);
    let drv_bytes = tokio::fs::read(&full_drv_path).await.with_context(|| {
        format!("read .drv {} for prefetch", full_drv_path)
    })?;
    let drv = parse_drv(&drv_bytes)
        .with_context(|| format!("parse .drv {} for prefetch", full_drv_path))?;

    let mut wanted: HashSet<String> = HashSet::new();
    // input_sources are plain store paths.
    for src in &drv.input_sources {
        wanted.insert(src.clone());
    }
    // input_derivations: read each input .drv to discover its output paths
    // (those output paths are what the daemon needs to actually build).
    ...
    // ── Filter to paths missing from the local store ─────────────────────────
    let mut missing: Vec<String> = Vec::new();
    for p in wanted {
        match store.has_path(&p).await {
            Ok(true) => {}
    ...
    if missing.is_empty() {
        debug!(build_id = %task.build_id, "all inputs already in local store; no prefetch needed");
        return Ok(());
    }
    ...
    // ── Ask the server which of these it can serve ───────────────────────────
    let cached_entries = updater
    ...
    // Split into "have presigned URL" vs "needs NarRequest" buckets.
    let mut by_url: Vec<CachedPath> = Vec::new();
    let mut by_request: Vec<CachedPath> = Vec::new();
    for cp in cached_entries {
        if !cp.cached {
            // Server doesn't have it either — daemon build will surface this.
            continue;
        }
        if cp.url.is_some() {
            by_url.push(cp);
    ...
    // ── NarRequest path (local-mode cache) ───────────────────────────────────
    let mut request_results: Vec<(String, Vec<u8>, CachedPath)> = Vec::new();
    if !by_request.is_empty() {
        let paths: Vec<String> = by_request.iter().map(|c| c.path.clone()).collect();
        let bytes_by_path = updater.request_nars(paths).await?;
        // Re-zip with metadata for import.
        let mut meta_by_path: std::collections::HashMap<String, CachedPath> =
            by_request.into_iter().map(|c| (c.path.clone(), c)).collect();
        for (path, bytes) in bytes_by_path {
            if let Some(meta) = meta_by_path.remove(&path) {
                request_results.push((path, bytes, meta));
            }
        }
    }

    // ── Presigned-URL path (S3-backed cache) ─────────────────────────────────
    let mut url_results: Vec<(String, Vec<u8>, CachedPath)> = Vec::new();
    if !by_url.is_empty() {
        let http = reqwest::Client::builder()
            .timeout(HTTP_DOWNLOAD_TIMEOUT)
            .build()
            .context("build reqwest client for presigned downloads")?;
        let mut futs = by_url
            .into_iter()
            .map(|cp| {
                let http = http.clone();
        ...
        while let Some(r) = futs.next().await {
            match r {
                Ok(triple) => url_results.push(triple),
                Err(e) => warn!(error = %e, "presigned NAR download failed; build may need to refetch"),
            }
        }
    }

    // ── Import everything into the local daemon, capped concurrency ──────────
    ...
    for (path, bytes, meta) in all_results {
        if imports.len() >= PREFETCH_CONCURRENCY {
            // Drain one before queueing the next.
            if let Some(r) = imports.next().await {
                if let Err(e) = r {
    ...
    let _ = strip_nix_store_prefix; // referenced for future use (signed-NAR variant)
    Ok(())
}
...
pub async fn import_received_nar(
    ...
    // ── Decompress ───────────────────────────────────────────────────────────
    let decompressed = decompress_zstd(&compressed_nar)
        .with_context(|| format!("zstd decompress failed for {}", store_path))?;

    // Sanity: the decompressed size must match what the metadata claims (if
    // supplied) so we don't silently accept a truncated transfer.
    if let Some(expected) = meta.nar_size
    ...
    // Sanity: re-hash and compare to the cache's claimed nar_hash so a
    // corrupted transfer can't silently pollute the local store.
    if let Some(claimed_nar_hash) = meta.nar_hash.as_deref() {
    ...
    // ── Build the path info the daemon expects ───────────────────────────────
    let path_base = store_path.strip_prefix("/nix/store/").unwrap_or(store_path);
    let path = StorePath::from_base_path(path_base)
        .map_err(|e| anyhow::anyhow!("invalid store path {}: {}", store_path, e))?;

    let info = build_unkeyed_path_info(store_path, meta, decompressed.len() as u64)?;
    let valid_info = ValidPathInfo {
        path: path.clone(),
        info,
    };

    // ── Stream into the daemon ───────────────────────────────────────────────
    let mut guard = store
        .pool()
        .acquire()
        .await
    ...
    // `&[u8]` implements both `AsyncRead` and `AsyncBufRead`, which is exactly
    // what `add_to_store_nar`'s `R: AsyncBufRead` source requires. Pin the
    // returned `ResultLog` and drain its log-message stream before awaiting
    // the future result, mirroring the pattern in `build_derivation`.
    ...
    while let Some(_msg) = logs.next().await {
        // Daemon log frames during import are noisy and not user-facing —
        // drop them. Errors arrive via the future result below.
    }
    logs.await
        .map_err(|e| anyhow::anyhow!("daemon add_to_store_nar({}) failed: {}", store_path, e))?;

    debug!(%store_path, bytes = compressed_nar.len(), "imported NAR into local store");
    Ok(())
}
...
fn build_unkeyed_path_info(
    ...
    let nar_hash = hash_any.into_hash().try_into().map_err(|e| {
        anyhow::anyhow!("convert nar_hash '{}' to NarHash: {}", nar_hash_str, e)
    })?;

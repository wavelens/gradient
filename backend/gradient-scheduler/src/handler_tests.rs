...
fn make_derivation(id: DerivationId, org_id: OrganizationId, path: &str) -> MDerivation {
    let stripped = gradient_core::executor::strip_nix_store_prefix(path);
...
fn make_state(db: sea_orm::DatabaseConnection) -> Arc<ServerState> {
    gradient_test_support::prelude::test_state(db)
}
...
async fn build_failed_appends_worker_error_to_log() {
    use std::sync::Arc;
    use gradient_test_support::prelude::{RecordingLogStorage, test_state_with_log_storage};
...
async fn build_output_inserts_build_product_rows() {
    ...
    let fake_bp = gradient_entity::build_product::Model {
...
async fn eval_job_completed_with_eval_errors_marks_eval_failed() {
    ...
    let eval_msg = gradient_entity::evaluation_message::Model {
        id: EvaluationMessageId::now_v7(),
        evaluation: eval_id,
        level: gradient_entity::evaluation_message::MessageLevel::Error,
...
async fn eval_result_creates_entry_points_for_project() {
    ...
    let db = MockDatabase::new(DatabaseBackend::Postgres)
        ...
        .append_query_results([vec![gradient_entity::entry_point::Model {

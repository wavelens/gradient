...
pub fn decrypt_ssh_private_key(
    ...
    let formatted_public_key = format_public_key(project, serve_url);
    let decrypted_private_key = format!("{}\n", decrypted_private_key);

    Ok((decrypted_private_key, formatted_public_key))
}

pub fn format_public_key(project: MProject, serve_url: &str) -> String {
    let hostname = serve_url
        ...
        .next()
        .unwrap_or(serve_url);
    format!("{} {}-{}", project.public_key, hostname, project.name)
}
...
mod tests {
    ...
    fn make_project(name: &str, public_key: &str) -> MProject {
        MProject {
            id: gradient_types::ids::ProjectId::nil(),
            name: name.to_string(),
    ...
    fn format_public_key_strips_https() {
        ...
        let result = format_public_key(project, "https://example.com");
    ...
    fn format_public_key_strips_path() {
        let project = make_project("myproject", "ssh-ed25519 AAAA");
        let result = format_public_key(project, "https://example.com/api/v1");
        assert_eq!(result, "ssh-ed25519 AAAA example.com-myproject");
    }
    ...
    fn decrypt_ssh_key_corrupt_base64_fails() {
        ...
        let mut project = make_project("o", "ssh-ed25519 AAAA");
        project.private_key = "!!!not-base64!!!".to_string();
        let result = decrypt_ssh_private_key(&path, project, "https://example.com");
        assert!(matches!(
            result,
            Err(SourceError::ProjectKeyDecoding { .. })
        ));
    }
    ...
    fn decrypt_ssh_key_plaintext_pem_rejected() {
        ...
        let mut project = make_project("o", "ssh-ed25519 AAAA");
        project.private_key = general_purpose::STANDARD.encode(pem);
        let result = decrypt_ssh_private_key(&path, project, "https://example.com");
    ...
    fn decrypt_ssh_key_plaintext_non_pem_rejected() {
        ...
        let path = f.path().to_string_lossy().to_string();
        let mut project = make_project("o", "ssh-ed25519 AAAA");
        project.private_key = general_purpose::STANDARD.encode(b"random garbage not a key");
        let result = decrypt_ssh_private_key(&path, project, "https://example.com");
        assert!(matches!(result, Err(SourceError::KeyDecryption { .. })));
    }
    ...
    fn generate_ssh_key_decrypts_to_openssh_pem() {
        ...
        let mut project = make_project("myproject", &pub_openssh);
        project.private_key = enc_priv;
        let (priv_pem, pub_formatted) =
            decrypt_ssh_private_key(&path, project, "https://example.com").expect("decrypt failed");
        assert!(priv_pem.starts_with("-----BEGIN OPENSSH PRIVATE KEY-----"));
        assert!(pub_formatted.starts_with(&pub_openssh));
        assert!(pub_formatted.ends_with(" example.com-myproject"));
    }
}

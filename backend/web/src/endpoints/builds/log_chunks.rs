...
type ChunkRow = entity::build_log_chunk::Model;

async fn load_chunk_rows(state: &ServerState, log_key: BuildId) -> WebResult<Vec<ChunkRow>> {
    Ok(entity::build_log_chunk::Entity::find()
        .filter(entity::build_log_chunk::Column::Build.eq(log_key))
        .order_by_asc(entity::build_log_chunk::Column::ChunkIndex)
        .all(&state.web_db)
        .await?)
}
...
pub async fn get_build_log_chunk(
    ...
    let row = entity::build_log_chunk::Entity::find()
        .filter(entity::build_log_chunk::Column::Build.eq(log_key))
        .filter(entity::build_log_chunk::Column::ChunkIndex.eq(index as i32))
        .one(&state.web_db)
        .await?

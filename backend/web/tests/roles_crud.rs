...
/// A custom role with `ManageWebhooks` set must satisfy a webhook-management
/// authorization gate, exercising the bitmask path through `access::has_permission`.
#[test]
fn custom_role_with_manage_webhooks_can_list_webhooks() {
    run(async {
        let session_id = SessionId::now_v7();
        let token = make_token(session_id);
        let custom_id = RoleId::now_v7();
        let custom_membership = organization_user::Model {
            id: OrganizationUserId::now_v7(),
            organization: org_id(),
            user: user_id(),
            role: custom_id,
        };
        let custom_role = role::Model {
            id: custom_id,
...
#[test]
fn custom_role_without_manage_webhooks_is_rejected() {
    run(async {
        let session_id = SessionId::now_v7();
        let token = make_token(session_id);
        let custom_id = RoleId::now_v7();

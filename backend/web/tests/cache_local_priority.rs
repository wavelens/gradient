...
fn cache_row(local_priority: Option<i32>) -> entity::cache::Model {
    entity::cache::Model {
        id: cache_id(),
...
fn build_server(cache: entity::cache::Model, peer: &str) -> TestServer {
    let cli = test_cli();
    ...
    let router = web::create_router(state).layer(MockConnectInfo(peer_addr));

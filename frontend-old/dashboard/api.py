...
def post_auth_basic_register(username, name, email, password):
    return get_client(
        ...
        "auth/basic/register",
        "POST",
        body={"username": username, "name": name, "email": email, "password": password},
    )
...
def post_auth_resend_verification(username):
    return get_client(
        None, "auth/resend-verification", "POST", body={"username": username}
    )
...
def patch_orgs_organization(
    request, organization=None, name=None, display_name=None, description=None
):
    body = {}
...
def delete_orgs_organization_users(request, organization, user):
    return get_client(
        request.user, f"orgs/{organization}/users", "DELETE", body={"user": user}
    )
...
def post_orgs_organization_subscribe_cache(request, organization, cache):
    return get_client(
        request.user, f"orgs/{organization}/subscribe-cache/{cache}", "POST"
    )


def delete_orgs_organization_subscribe_cache(request, organization, cache):
    return get_client(
        request.user, f"orgs/{organization}/subscribe-cache/{cache}", "DELETE"
    )
...
def put_projects(
    ...
    return get_client(
        ...
        body={
            ...
            "evaluation_wildcard": evaluation_wildcard,
        },
    )
...
def patch_projects_project(
    ...
    description=None,
    repository=None,
    evaluation_wildcard=None,
):
    return get_client(
        request.user,
        f"projects/{organization}/{project}",
        "PATCH",
        body={
            "name": name,
            "display_name": display_name,
...
def delete_projects_project_active(request, organization, project):
    return get_client(
        request.user, f"projects/{organization}/{project}/active", "DELETE"
    )


def post_projects_project_check_repository(request, organization, project):
    return get_client(
        request.user, f"projects/{organization}/{project}/check-repository", "POST"
    )


def post_projects_project_evaluate(request, organization, project):
    return get_client(
        request.user, f"projects/{organization}/{project}/evaluate", "POST"
    )
...
def post_evals_evaluation_abort(request, evaluation):
    return get_client(
        request.user, f"evals/{evaluation}", "POST", body={"method": "abort"}
    )
...
def post_servers_server_check_connection(request, organization, server):
    return get_client(
        request.user, f"servers/{organization}/{server}/check-connection", "POST"
    )
...
def patch_caches_cache(
    request, cache, name=None, display_name=None, description=None, priority=None
):
    return get_client(
        request.user,
        f"caches/{cache}",

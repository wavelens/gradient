...
pub async fn handle(cmd: Commands) {
    match cmd {
        ...
        Commands::Edit {
            ...
            let current_server =
                servers::get_server(get_request_config(load_config()).unwrap(), organization.clone(), name.clone())
                    .await
                    .map_err(|e| {
                        eprintln!("{}", e);
                        exit(1);
            ...
            let input_fields = vec![
                ...
                ("Port", port.map(|p| p.to_string()).or(Some(current_server.port.to_string()))),

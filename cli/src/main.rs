...
pub async fn main() -> std::io::Result<()> {
    ...
    match cli.cmd {
        ...
        MainCommands::Organization { cmd, organization_id } => {
            match cmd {
                OrganizationCommands::Create { name, description } => {
                    let name = match name {
                        Some(name) => name,
                        None => ask_for_input("Name")
                    };
                    ...
                    let res = request::create_organization(load_config(), name, description).await.map_err(|e| {
                        eprintln!("{}", e);
                        exit(1);
                    }).unwrap();
        ...
        MainCommands::Project { cmd } => {
            match cmd {
                ProjectCommands::Create { organization_id, name, description, repository } => {
                    let organization_id = match organization_id {
                        Some(id) => id,
                    ...
                    let res = request::create_project(load_config(), organization_id, name, description, repository).await.map_err(|e| {
                        eprintln!("{}", e);
                        exit(1);
                    }).unwrap();

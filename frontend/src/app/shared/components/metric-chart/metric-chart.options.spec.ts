...
import { buildMetricChartOption, CHART_THEME } from './metric-chart.options';
...
describe('buildMetricChartOption cartesian types', () => {
  it('renders area as a smooth line series with a gradient fill', () => {
    const opt = buildMetricChartOption({ type: 'area', series: one, categories: cats });
  ...
  it('renders line without an area fill', () => {
    const opt = buildMetricChartOption({ type: 'line', series: one, categories: cats });
    const s = (opt.series as any[])[0];
    expect(s.type).toBe('line');
  ...
  it('renders bar as a bar series with the category axis on x', () => {
    const opt = buildMetricChartOption({ type: 'bar', series: one, categories: cats });
  ...
  it('swaps the axes for a horizontal bar', () => {
    const opt = buildMetricChartOption({ type: 'bar', series: one, categories: cats, horizontal: true });
  ...
  it('keeps null gaps in the data instead of coercing them to zero', () => {
    const opt = buildMetricChartOption({ type: 'line', series: [{ name: 'a', data: [1, null, 3] }], categories: cats });
...
describe('buildMetricChartOption presentation', () => {
  it('passes colors through', () => {
    const opt = buildMetricChartOption({ type: 'line', series: one, categories: cats, colors: ['#abc123'] });
    expect(opt.color).toEqual(['#abc123']);
  });

  it('shows a legend only when there is more than one series', () => {
    expect((buildMetricChartOption({ type: 'line', series: one, categories: cats }).legend as any).show).toBe(false);
    expect((buildMetricChartOption({ type: 'line', series: two, categories: cats }).legend as any).show).toBe(true);
  });
  ...
  it('sets the y axis title when given', () => {
    const opt = buildMetricChartOption({ type: 'line', series: one, categories: cats, yAxisTitle: 'seconds' });
    expect((opt.yAxis as any).name).toBe('seconds');
  });

  it('uses the dark surface theme colors', () => {
    const opt = buildMetricChartOption({ type: 'line', series: one, categories: cats });
    expect((opt.xAxis as any).axisLabel.color).toBe(CHART_THEME.text);
...
describe('buildMetricChartOption radar', () => {
  it('builds indicators from the categories and emits a radar series', () => {
    const opt = buildMetricChartOption({ type: 'radar', series: one, categories: cats });
...
describe('buildMetricChartOption heatmap', () => {
  ...
  it('derives the x categories from the first series points', () => {
    const opt = buildMetricChartOption({ type: 'heatmap', series: bands });
  ...
  it('survives an empty series list', () => {
    const opt = buildMetricChartOption({ type: 'heatmap', series: [] });
    expect((opt.series as any[])[0].data).toEqual([]);
...
describe('buildMetricChartOption dual axis', () => {
  ...
  it('emits two value axes with the second on the right', () => {
    const y = buildMetricChartOption(dual).yAxis as any[];
  ...
  it('binds a right-axis series to the second axis', () => {
    const s = buildMetricChartOption(dual).series as any[];
  ...
  it('formats each tooltip row with the formatter of its own axis', () => {
    const tooltip = buildMetricChartOption(dual).tooltip as any;
